...
object projet {
  ...
  def salarySum(employees: List[Employee]): Double = employees.foldLeft(0.0)(_ + _.salary)
  // employees.foldLeft(0.0){ (accumulator, employee) => accumulator + employee.salary }
  ...
  def addressOf(users: List[User]): List[String] = 
    users.foldLeft(List[String]())(_ :+ _.address)
  ...
  def average(values: Iterator[Double]): Option[Double] = {
    ...
    else Option(values.foldLeft((0.0, 1)) { case ((avg, idx), next) => (avg + (next - avg)/idx, idx + 1) }._1)
  }
